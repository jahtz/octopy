...
@click.option(
     "-g", "--gt", "ground_truth",
     help="One or more ground truth PageXML files. Use quotes to enclose glob patterns (e.g., \"*.xml\").",
...
@click.option(
     "-e", "--eval", "evaluation",
     help="One or more optional evaluation PageXML files. Use quotes to enclose glob patterns (e.g., \"*.xml\").",
     type=click.Path(exists=False, dir_okay=False, file_okay=True, resolve_path=True),
     callback=util.callback_paths_optional,
     multiple=True
)
@click.option(
...
@click.option(
     ...
     default=SEGMENTATION_HYPER_PARAMS['padding'], 
     show_default=True, 
     nargs=2
)
@click.option(
...
@click.option(
     ...
     type=click.FLOAT, 
     default=SEGMENTATION_HYPER_PARAMS["lrate"], 
     show_default=True
)
@click.option(
     "--momentum", "momentum",
     ...
     type=click.FLOAT, 
     default=SEGMENTATION_HYPER_PARAMS["momentum"], 
     show_default=True
)
@click.option(
     ...
     type=click.FLOAT, 
     default=SEGMENTATION_HYPER_PARAMS["weight_decay"], 
     show_default=True
)
@click.option(
     "--schedule", "schedule",
...
@click.option(
     ...
     type=click.INT, 
     default=SEGMENTATION_HYPER_PARAMS["completed_epochs"], 
     show_default=True
)
@click.option(
     "--augment", "augment",
...
@click.option(
     ...
     type=click.FLOAT, default=SEGMENTATION_HYPER_PARAMS["rop_factor"], 
     show_default=True
)
@click.option(
...
@click.option(
     ...
     type=click.INT, 
     default=SEGMENTATION_HYPER_PARAMS["warmup"], 
     show_default=True
)
@click.option(
     "--precision", "precision",
...
@click.option(
     "--suppress-baselines", "suppress_baselines",
     help="Disable baseline segmentation training.",
     type=click.BOOL, 
     is_flag=True
)
@click.option(
     "-vr", "--valid-regions", "valid_regions",
     help="Comma-separated list of valid regions to include in the training. Applied before region merging.",
     type=click.STRING, 
     callback=util.callback_validate
)
@click.option(
     "-vb", "--valid-baselines", "valid_baselines",
     help="Comma-separated list of valid baselines to include in the training. Applied before baseline merging.",
     type=click.STRING, 
     callback=util.callback_validate
)
@click.option(
     "-mr", "--merge-regions", "merge_regions",
...
@click.option(
     "-mb", "--merge-baselines", "merge_baselines",
